...
BOOST_AUTO_TEST_CASE(testLogStreamLong)
{
  ...
  for (int i = 0; i < 399; ++i)
  {
    ...
    BOOST_CHECK_EQUAL(buf.avail(), 4 * 1024 - 10*(i+1));
  }

  os << "abcdefghi ";
  BOOST_CHECK_EQUAL(buf.length(), 4000);
  BOOST_CHECK_EQUAL(buf.avail(), 4 * 1024 - 4000);

  os << "abcdefghi";
  BOOST_CHECK_EQUAL(buf.length(), 4009);
  BOOST_CHECK_EQUAL(buf.avail(), 4 * 1024 - 4009);
}